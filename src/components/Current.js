    ...
    return (
			...
			<div className="flex flex-col md:flex-row justify-center items-center md:h-[88%] h-auto w-full primaryBColor rounded-md p-2">
				...
				<div className="flex flex-col w-2/3 pl-2 justify-center items-center md:justify-start md:items-start order-2 md:order-1">
					...
					<p className="font-light mb-3">{weather.weather ? <>{weather.weather.map(x=>{return x.main})}</>:<div className="lds-dual-ring"></div>}</p>
					<div className="w-full h-auto flex items-center justify-center md:justify-start text-center">
						<div className="bg-white px-1 py-2 rounded-lg mr-2">
							<p>Pressure</p>
							<p>{weather ? <>{weather.pressure}</>:<div className="lds-dual-ring"></div>}hpa</p>
						</div>
						<div className="bg-white px-1 py-2 rounded-lg mr-2">
							<p>Humidity</p>
							<p>{weather ? <>{weather.humidity}</>:<div className="lds-dual-ring"></div>}%</p>
						</div>
						<div className="bg-white px-1 py-2 rounded-lg mr-2">
							<p>Wind</p>
							<p>{weather.wind_speed ? <>{weather.wind_speed.toFixed(1)}</>:<div className="lds-dual-ring"></div>}m/hr</p>
						</div>
					</div>
				</div>
				{/* icon cnt */}
